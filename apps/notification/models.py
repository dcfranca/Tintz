...
class NoticeType(models.Model):
    ...
    to_follow = models.BooleanField( _('to_follow'))
...
class NoticeSetting(models.Model):
    ...
    class Meta:
        ...
        verbose_name_plural = _("notice tintzsettings")
...
class NoticeManager(models.Manager):
    ...
    def notices_for(self, user, archived=False, unseen=None, on_site=None,  notice_type=None,  to_follow=False):
        """
        ...
        qs.filter( notice_type__gt=5 )
        return qs

    def unseen_count_for(self, user,  notice_type=None):
        """
        returns the number of unseen notices for the given user but does not
        mark them seen
        """
        if notice_type is not None:
            count = self.filter(user=user, unseen=True,  notice_type=notice_type).count()
        else:
            count = self.filter(user=user, unseen=True, notice_type__gt=5).count()
        return count
    
    def unseen_follow_count_for(self,  user):
        """
        returns the number of unseen notifications sent to followers
        """
        notice_types = NoticeType.objects.filter(to_follow=True)
        count = 0
        for notice_type in notice_types:
            count += self.filter( user=user,  notice_type = notice_type,  unseen=True ).count()
...
def create_notice_type(label, display, description, default=2, verbosity=1,  to_follow=False):
    """
    ...
    try:
        ...
        NoticeType(label=label, display=display, description=description, default=default,  to_follow=to_follow).save()
        if verbosity > 1:
